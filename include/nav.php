<?php 
    class NavLink{
        ...
        public function __construct($file, $target, $title, $text) {
            $this->file = $file;
            $this->target = $target;
            $this->text = $text;
            $this->title = $title;
        ...
        public function link_navigasi() {
            return "<div class=\"col-md-2\">
                        <a href=\"{$this->file}\" class=\"btn btn-dark\" target=\"{$this->target}\" title=\"$this->title\">{$this->text}</a>
                    </div>";
        }
    }
    ...
    $pos = new NavLink("pos/", "_blank", "POS (Point Of Sales)", "<i class=\"bi bi-calculator\"></i>");
