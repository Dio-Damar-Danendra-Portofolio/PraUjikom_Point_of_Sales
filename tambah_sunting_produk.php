<?php 
    ...
    if(isset($_POST['tambah'])){
        ...
        if (!in_array($ekstensi, $extension)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO products (name, price, image, category_id, stock, description, is_active) 
            VALUES ('$name', '$price', '$image', '$category_id', '$stock', '$description', '$is_active');");
        ...
        if(isset($_POST['simpan'])){
            if (isset($_GET['id-produk'])) {
                ...
                if (!in_array($ekstensi, $extension)) {
                    ...
                    $update = mysqli_query($koneksi, "UPDATE products SET name = '$name', price = '$price', image = '$image', category_id = '$category_id', stock = '$stock', description = '$description', is_active = '$is_active' WHERE id = '$id';");
...
<main class="bg-danger pt-5 pb-5" style="margin-top: 140px; margin-bottom: 100px;"> 
        ...
        <div class="container">
                ...
                <div class="row">
                        <div class="col-md-6">
                            <label for="name" class="form-label fw-bold fs-30 text-black">Nama Produk (wajib diisi): </label>
                            <input type="text" name="name" id="name" class="form-control" required value="<?php echo isset($_GET['id-produk']) ? $row_edit['name'] : ''; ?>">
                        </div> 
                        <div class="col-md-6">
                            <label for="price" class="form-label fw-bold fs-30 text-black">Harga Produk (wajib diisi): </label>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <label for="image" class="form-label fw-bold fs-30 text-black">Foto Produk (wajib diisi): </label>
                        <input type="file" name="image" id="image" class="form-control" <?php echo isset($_GET['id-produk']) ? '' : 'required'; ?>>
                        <?php if (isset($_GET['id-produk'])) { ?>
                            <small class="text-black">Nama file sebelumnya: <?php echo $row_edit['image']; ?></small><br>
                            <img src="uploads/products/<?php echo $row_edit['image']; ?>" class="mt-4" width="200" alt="Foto tidak tersedia">
                        <?php } ?>
                    </div>
                    <div class="col-md-6">
                        <label for="category_id" class="form-label fw-bold fs-30 text-black">Kategori Produk (wajib dipilih): </label>
                ...
                <div class="row">
                    <div class="col-md-6">
                    <label class="form-label fw-bold fs-30 text-black" for="stock">Stok: </label>
                    <input type="number" id="stock" name="stock" class="form-control" <?php echo isset($_GET['id-produk']) ? $row_edit['stock'] : ''; ?>>
                    </div>  
                    <div class="col-md-6">
                    <label class="form-label fw-bold fs-30 text-black" for="description">Deskripsi Produk: </label>
